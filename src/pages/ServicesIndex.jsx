...
function ServiceCard({ service }) {
  ...
  return (
    <motion.div variants={cardVariants}>
      <Link
        to={href}
        className="group block p-6 border border-primary-30 hover:bg-primary-30/10 transition-all duration-300 h-full"
      >
    ...
    <>
      ...
      <section className="bg-secondary py-24 md:py-32">
        <div className="mx-auto max-w-7xl px-6 md:px-12 lg:px-16">
          <CategorySection label="Security Services" services={securityServices} />
          <CategorySection label="Cloud Financial Services" services={finopsServices} />
          <CategorySection label="Proprietary Product" services={productServices} />
        </div>
      </section>
    </>
  );
