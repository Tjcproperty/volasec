...
export default function RelatedServices({ slugs, dark = false }) {
      ...
      {related.map((service) => {
        ...
        return (
          <motion.div key={service.slug} variants={cardVariants}>
            <Link
              to={href}
              className={`block p-6 border ${border} ${hoverBg} transition-all duration-300 group`}
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3
                    className={`text-lg font-bold ${textPrimary} mb-2 group-hover:text-primary transition-colors`}
                  >
                    {service.title}
                  </h3>
                  <p
                    className={`text-sm ${textSecondary} font-light leading-relaxed mb-4`}
                  >
                    {service.shortDescription}
                  </p>
                  <span className="inline-flex items-center gap-2 text-xs font-bold text-primary tracking-wider uppercase">
                    Learn More
